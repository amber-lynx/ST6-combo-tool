...
// ネスト表示対応
function showFollow(list, level){
    const container = document.getElementById("followupsText");
    if(level === 1) container.innerHTML = ""; // 最初の階層ならリセット

    list.forEach(f=>{
        let cmd = f.cmd?`【${f.cmd}】`:"";
        const btn = document.createElement("button");
        btn.innerText = `${f.name}${cmd}`;
        btn.style.marginLeft = `${(level-1)*20}px`; // インデント
        ...
        // 子階層はクリック時に展開
        if(f.followups){
            btn.addEventListener("click", ()=>showFollow(f.followups, level+1));
        }
    });
}
